...
$p = mysql_query("CREATE TABLE IF NOT EXISTS resource(rid INT($max_resource) PRIMARY KEY AUTO_INCREMENT,r_name VARCHAR(40) NOT NULL,location VARCHAR(40) NOT NULL,facilities VARCHAR(400),pic VARCHAR(100) DEFAULT 'img/resource/default.jpg' NOT NULL,description VARCHAR(200) NOT NULL,ctrl_o VARCHAR(100) NOT NULL)");
if(!$p){
...
$p = mysql_query("CREATE TABLE IF NOT EXISTS request(req_id INT($max_req) PRIMARY KEY AUTO_INCREMENT,title VARCHAR(100) NOT NULL,description VARCHAR(200),s_date DATE NOT NULL, e_date DATE NULL,s_time TIME NOT NULL, e_time TIME NOT NULL,entrydate TIMESTAMP NOT NULL,uid INT($max_user) NOT NULL, rid INT($max_resource) NOT NULL,status VARCHAR(100) DEFAULT 'pending',seen INT(1) DEFAULT 0 NOT NULL,FOREIGN KEY(uid) REFERENCES user_details(uid) ON DELETE CASCADE,FOREIGN KEY(rid) REFERENCES resource(rid))");
if(!$p){
...
$p = mysql_query("CREATE TABLE IF NOT EXISTS request_status(remark VARCHAR(200),status VARCHAR(100) DEFAULT 'not_recieved',status_timestamp TIMESTAMP,uid INT($max_user) NOT NULL,req_id INT($max_req) NOT NULL,seen INT(1) DEFAULT 0 NOT NULL,PRIMARY KEY(req_id,uid),FOREIGN KEY(uid) REFERENCES user_details(uid),FOREIGN KEY(req_id) REFERENCES request(req_id) ON DELETE CASCADE)");
if(!$p){
